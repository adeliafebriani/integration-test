...
describe('End-to-End Test: Token and AnotherContract', function () {
    ...
    before(async function() { // before method to calculate all the total balance
    ...
    it("Owner should transfer token to AnotherContract", async function () {
        ...
        const contractBalance = await token.balanceOf(anotherContract.getAddress());
    ...
    it("User1 should fail to transfer token without enough balance", async function () {
        await expect(anotherContract.connect(user).receiveTokens(ethers.parseEther('100')))
            .to.be.revertedWith('Insufficient balance in sender.');
    });

    it("User2 transfer token after receiving from the Owner", async function () {
        ...
        const user2Balance = token.balanceOf(user2.address);
        const anotherContractBalance = token.balanceOf(await anotherContract.getAddress());
